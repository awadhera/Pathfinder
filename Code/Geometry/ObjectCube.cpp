...
void Cube::setTexture(TGA &tgaObjIn)
{
	textureSet = true;
	tgaObj = tgaObjIn;
	createTexCoord();
...
void Cube::createTexCoord()
{
	float inc_tex = 1.0f/subd;
	...
	for(int k = 0; k < 6; ++k)
	{
		for(unsigned short j = 0;j <= subd; ++j)
		{
			for(unsigned short i = 0;i <= subd; ++i)
			{
				g_VertexArray[k*(((subd+1)*(subd+1)))+(j*(subd+1)+i)].tc.s = (inc_tex*i);
				g_VertexArray[k*(((subd+1)*(subd+1)))+(j*(subd+1)+i)].tc.t = (inc_tex*j);
			}
		}
	}
}
