...
void Cylinder::setTexture(TGA &tgaObjIn)
{
	textureSet = true;
	tgaObj = tgaObjIn;
	createTexCoord();
...
void Cylinder::createTexCoord()
{
	float inc_tex_slice = 1.0f/slices;
	float inc_tex_stack = 1.0f/stacks;
	float angle_d = 360.0f/slices;
	float angle_r = (angle_d*M_PI)/180;
	float inc_y = (mSpace.limit_r-mSpace.limit_l)/stacks;
	float radius = (mSpace.limit_r-mSpace.limit_l)/2;
	...
	{	
		...
		for(j = 0,k = 0;j <= slices; ++j,k+=angle_r)
		{
			if(j==slices)
			{
				g_VertexArray[(i*(slices+1))+j].tc.s = g_VertexArray[i*(slices+1)].p.x;
				g_VertexArray[(i*(slices+1))+j].tc.t = g_VertexArray[i*(slices+1)].p.y;
			}
			else
			{
				g_VertexArray[(i*(slices+1))+j].tc.s = inc_tex_stack*i;
				g_VertexArray[(i*(slices+1))+j].tc.t = (i*(slices+1))+j;
			}
		}
	}
}

void Cylinder::uploadTex()
{
